...
{% highlight xml %}<DeployIisAppPath>Default Web Site/</DeployIisAppPath>{% endhighlight %}  

This is a per configuration setting, so you must add it to each configuration.  For example:

{% highlight xml %}<PropertyGroup Condition="'$(Configuration)|$(Platform)' == 'UAT|AnyCPU'">
    <OutputPath>bin\</OutputPath>
    <DeployIisAppPath>Default Web Site/</DeployIisAppPath>
</PropertyGroup>{% endhighlight %}
